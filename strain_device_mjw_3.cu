...
extern "C" __device__ bool GetScatteringOmegas( float &fOmegaRes1, float &fOmegaRes2,
...
extern "C" __device__ void Intersection(int &iJ, int &iK, bool &bMask,
...
extern "C" __device__ void JK2Window(int &iX, int &iY, int &iOffset, bool &bMask,
...
extern "C" __global__ void Simulate_for_Strain(int *aiX, int *aiY, int *aiOffset, bool *abMask, bool *abtrueMask,
        float fx, float fy, const float* __restrict__ afDetInfo, const float *afDistortion,
        const int* __restrict__ iWhichOmega,
        int iNumD, int iNumG, float fBeamEnergy, int iNumFrame, const int* __restrict__ Lim, int LimSize, float OmegaInterval,float *tfG){
    /*
    ...
    for(int ii=0;ii<3;ii++){
        for(int jj=0;jj<3;jj++){
            afRotatedG[ii]+=afDistortion[j*9+ii*3+jj]*tfG[i*3+jj];//tex1Dfetch<float>(tfG, i*3+jj);
        }
    }
...
extern "C" __global__ void Simulate_for_Pos(int *aiX, int *aiY, int *aiOffset, bool *abMask, bool *abtrueMask,
    ...
    for(int ii=0;ii<3;ii++){
        for(int jj=0;jj<3;jj++){
            afRotatedG[ii]+=afDistortion[j*9+ii*3+jj]*tfG[i*3+jj];
        }
    }
    GetScatteringOmegas( fOmegaRes1, fOmegaRes2, fTwoTheta, fEta, fChi , afRotatedG,fBeamEnergy);
    if(iWhichOmega[i]==1){
...
extern "C" __global__ void Hit_Score(float *afscore,
...
extern "C" __global__ void KL_diff(float *afKLdiff,
        const int* __restrict__ aiX, const int*  __restrict__ aiY, 
        const int* __restrict__  aiOffset, const bool*  __restrict__ abMask, const bool*  __restrict__ abtrueMask,
    ...
    if(i<iNumD){
        ...
        for(int jj=0;jj<iNumG;jj++){
            float ftmp;
            unsigned int tmpIdx=i*iNumG+jj;
...
extern "C" __global__ void L1_diff(float *afL1diff,
        const int* __restrict__ aiX, const int*  __restrict__ aiY, 
        const int* __restrict__  aiOffset, const bool*  __restrict__ abMask, const bool*  __restrict__ abtrueMask,
...
/*extern "C" __global__ void display_rand(float* afRandom, int iNRand){
        int i = blockIdx.x*blockDim.x + threadIdx.x;
        printf("=%d=",i);
        if (i<iNRand){
        printf(" %f ||", afRandom[i]);
...
extern "C" __global__ void euler_zxz_to_mat(float* afEuler, float* afMat, int iNAngle){
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if(i<iNAngle){
...
extern "C" __device__ void d_euler_zxz_to_mat(float* afEuler, float* afMat){
...
extern "C" __global__ void mat_to_euler_ZXZ(float* afMatIn, float* afEulerOut, int iNAngle){
    /*
...
extern "C" __global__ void rand_mat_neighb_from_euler(float* afEulerIn, float* afMatOut, float* afRand, float fBound){
...
extern "C" __device__ void mat3_transpose(float* afOut, float* afIn){
    /*
...
extern "C" __device__ void mat3_dot(float* afResult, float* afM0, float* afM1){
    /*
...
extern "C" __global__ void misorien(float* afMisOrien, float* afM0, float* afM1, float* afSymM){
    /*
...
extern "C" __device__ void d_misorien(float& fMisOrien, float* afM0, float* afM1, float* afSymM){
        /*
    * calculate the misorientation betwen afM0 and afM1
    * fMisOrien: 1
    * afM0: 9
